...
def evaluate_segmentation_datalist(datalist, num_classes):
    print("Starting Evaluation of Results!")
    hist = np.zeros((num_classes, num_classes))

    gt_id_set = set()
    for step, data in enumerate(datalist):
        gt_flatten = data[0]
        pred_flatten = data[1]
        gt_id_set.update(np.unique(gt_flatten).tolist())
        hist += fast_hist(gt_flatten, pred_flatten, num_classes)
